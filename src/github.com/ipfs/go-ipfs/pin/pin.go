...
func (p *pinner) Pin(ctx context.Context, node ipld.Node, recurse bool) error {
	...
	c := node.Cid()
    //fmt.Println("loong pinner Pin: cid %s, recurse: %b", c.String(), recurse)
	if recurse {
...
func LoadPinner(d ds.Datastore, dserv, internal ipld.DAGService) (Pinner, error) {
	...
	//fmt.Println("loong LoadPinner: rootKeyBytes = ", rootKeyBytes)

	rootCid, err := cid.Cast(rootKeyBytes)
	if err != nil {
		return nil, err
	}
	//fmt.Println("loong rootCid: ", rootCid.String())
...
func (p *pinner) Flush() error {
	...
	//fmt.Println("loong pinner.Flush")
	ctx := context.TODO()
